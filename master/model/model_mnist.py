...
def main_fun(args, ctx):
  ...
  def rdd_generator():
    while not tf_feed.should_stop():
      ...
      if len(batch) > 0:
        example = batch[0]
        input = np.array(example[0]).astype(np.float32) / 255.0
        input = np.reshape(input, (28, 28, 1))
...
if __name__ == "__main__":
  ...
  parser.add_argument("--images_labels", help="path to MNIST images and labels in parallelized format", default = 'data/mnist/tfr/train/')
