...
resource "openstack_networking_floatingip_v2" "float_ip_master" {
  pool = var.public_network
}

#resource "openstack_networking_floatingip_v2" "float_ip_worker" {
#  count = 2
#  pool = var.public_network
#}

resource "openstack_compute_floatingip_associate_v2" "fip_master" {
  floating_ip = openstack_networking_floatingip_v2.float_ip_master.address
  instance_id = openstack_compute_instance_v2.flatcar_master.id
}

#resource "openstack_compute_floatingip_associate_v2" "fip_worker" {
#  count = 2
#  floating_ip = openstack_networking_floatingip_v2.float_ip_worker[count.index].address
#  instance_id = openstack_compute_instance_v2.flatcar_worker[count.index].id
#}
...
resource "openstack_networking_secgroup_rule_v2" "basic_cert" {
    direction = "ingress"
    ethertype = "IPv4"
    protocol = "tcp"
    port_range_min = 8080
...
resource "openstack_networking_secgroup_rule_v2" "basic_kube" {
    direction = "ingress"
    ethertype = "IPv4"
    protocol = "tcp"
    port_range_min = 6443
