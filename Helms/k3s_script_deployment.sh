...
helm --kube-insecure-skip-tls-verify repo add grafana https://grafana.github.io/helm-charts
kubectl --insecure-skip-tls-verify  create namespace monitoring
helm install --kube-insecure-skip-tls-verify grafana grafana/grafana --namespace monitoring
kubectl  --insecure-skip-tls-verify get secret --namespace monitoring grafana -o jsonpath="{.data.admin-password}" | base64 --decode ; echo
export POD_NAME=$(kubectl --insecure-skip-tls-verify get pods --namespace monitoring -l "app.kubernetes.io/name=grafana,app.kubernetes.io/instance=grafana" -o jsonpath="{.items[0].metadata.name}")
kubectl --insecure-skip-tls-verify --namespace monitoring port-forward $POD_NAME 3000
